...
## Gladiator
강화 플라스틱-스프링 짐벌 방식 베이스.

## Gunfighter
캠 짐벌 및 댐퍼 방식 베이스. 매우 컴팩트한 형상으로 거치대와 함께 사용해 다리 사이로 넣을 때 특히 편안하다. 가장 발전된 댐퍼 매커니즘을 가지고 있어 매우 부드럽게 설정해도 큰 센터 데드존이 필요하지 않다.

## MCG Ultimate
알루미늄제 Su-57 레플리카 그립. 5개의 5-way 와 아날로그 브레이크 레버, 아날로그 플립 트리거 등 많은 버튼을 가지고 있다.
...
### 가상 토브레이크
T-Rudder 에는 토브레이크가 없다. 하지만 다른 VKB 기기를 가지고 있다면, T-Rudder에서 나오는 RJ45 포트를 VKB 기기 또는 블랙박스 RJ45 포트에 연결해 디퍼런설 브레이크를 시뮬레이션 할 수 있다. 특히 아날로그 브레이크 레버를 가지고 있는 MCG Ultimate 와 조합하면 완벽한 브레이크 시스템을 만들 수 있다.
