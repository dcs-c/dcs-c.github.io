...
해당 포드를 사용하면 상대 기체의 레이더 성능에 따라 다르지만 잍정 거리 이상에서는 락을 강제로 풀어버려 미사일 발사를 불가능하게 만든다. 그러나 그 일정 거리 이내로 들어 온 경우, 기만이 되지 않거나 오려 전파때문이 미사일과 상대 레이더에 더욱 잘 잡히는 걍우가 있다. 그러니 교전거리가 짧다면 ECM은 잠시 까 두도록 하자.
