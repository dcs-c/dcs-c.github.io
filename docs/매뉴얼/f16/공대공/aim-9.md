...
Enable switch를 누르면 시커가 시야를 따라 움직이다 열원이 포착되면 자동으로 락온이 된다는 점을 이용해 저고도 근접에서 적을 색적할 때 레이더를 꺼 상대 RWR을 건드리지 않는 상태에서 레이더 대용으로 사용이 가능하다.

여담으로, 간혹 초근접에서 미사일이 스쳐지나가서 피한 줄 알았던 AIM-9X 가 180도 돌아서 다시 쫒아오는 살벌한 상황도 종종 나온다.
