...
또한 HOJ 모드로 발사된 미사일은 재밍을 뚫고 추적하는게 아니라, 정확한 거리와 고도 정보 없이 단순히 재밍 신호의 방향을 추적하기 때문에 비행 경로가 효율적이지 못하고 오로지 목표에게 직선으로 날아간다. 미사일이 목표까지 직선으로 날아간다면 거리가 짧아져서 더 잘 맞을거라 생각 할 수 있지만, 실상은 직선거리로 간다면 포물선으로 가는 것 보다 공기 밀도가 상대적으로 높은 곳에서 오래 비행하기 때문에 미사일이 더 빠르게 멍청해진다.

물론 당신이 상대가 쐈을 가능성을 놓치고 방심하고 있다 미사일 경보가 울렸다면 에너지고 나발이고 재밍 신호를 쫒아온 미사일에 따뜻해 질 가능성이 높다. 그러니 재머를 켰다고 안심하지 말고 전술적으로 잘 사용 하도록 하자.
...
상대가 하드락을 걸지 않고 TWS로 트레킹 한다면 재머는 켜지지 않는다(...!) 

공대공 PVP에서는 재머가 의미가 있을 정도로 장거리에서 굳이 STT 하드 락을 잡는 사람은 없다고 봐도 무방하므로 PVP에서는 감히 쓸모가 없는 모드라고 단언할 수 있다.
...
1번 모드와 마찬가지로 상대가 하드락을 걸지 않고 TWS로 트레킹 한다면 재머는 켜지지 않는다(...!) 
...
재머를 필요할때 켜고 끌 수 있어 PVP에서 유일하게 쓸모가 있다고 할 수 있는 모드이다.
...
ECM은 F16이 고고도 BVR을 할 때, F14나 F15 같은 레이더 사거리가 훨씬 길고 에너지도 높은 기체들을 상대로 비벼볼 수 있게 만들어준다. 고고도에서 에너지는 절대로 꿀리지 않지만, 레이더 출력의 한계로 인해 에너지를 100% 활용하지 못하는 F16은 훨씬 멀리서 미사일을 발사하고 크랭킹 할 수 있는 F15 같은 기체를 이길 수가 없다. 그러나 ECM을 활용한다면 강제로 15의 레이더 사거리를 제한 해 버릴 수 있기 때문에 유리하지는 않아도 적어도 불리했던 레이더 격차를 줄이고 교전거리를 짧아지게 만들 수 있다.

이때, HOJ로 미사일이 날아올 가능성을 고려하여야 한다. HOJ로 쏘면 미사일이 비효율적인 경로로 날아오지만, 압도적인 에너지를 가진 기체가 쏘면 효율이고 나발이고 깡에너지로 공기저항을 뚫고 날아온 미사일에 맞아 뒤질 수 있으니 주의하도록 하자. 애초에 5만 피트정도에서 로프트를 하여 쏜 미사일은 포물선이 아닌 직선으로 날아가도 상대 고도만 충분히 높다면 에너지 손실이 적다.
...
구시대의 산물. ALQ-184와 비교하면 전체적인 성능이 크게 뒤쳐지니 굳이 장착할 메리트가 없.....어야 하지만 2026-1월 기준으로 코드를 뜯어본 결과 현재로써는 ALQ-184 와 같은 모듈을 사용하는 것으로 확인되었다.(즉, 성능 차이가 없다.)
