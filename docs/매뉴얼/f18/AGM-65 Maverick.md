...
>> 3. 월아이에서도 초록색 직사각형으로 렌즈가 어디까지 돌아갈 수 있는지 나타내주었다면 매버릭은 해당 문양에서 가운데 X자 표시가 원 테투리 밖으로 이동되면 더이상 매버릭이 레이저를 따라갈 수 없다는 뜻이다 따라서 투하할땐 목표로 하고 있는 대상과 고도차가 심하게 나면 날수록 기수를 내려줘야된다
...
매버릭은 자체 추진체가 있는 미사일이지만 선회력이 나빠 적과의 거리가 1마일인데 고도가 1만 정도인 상태에서 수평비행을 하며 발사하게 되면 아무리 레이징 중이거나 매버릭 자체 IR 시커가 적을 포착해도 선회가 잘 되지 않아 엉뚱한곳으로 떨어지니 어느정도 기수가 적을 향하도록 유지해줘야한다 한마디로 허드에 시커가 바라보는곳을 나타내는 역삼각형이 보여야 한다
