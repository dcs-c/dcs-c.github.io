...
필자 또한 그러한 날이 있었다. 그래서 어서 즐기고픈 마음을 가지고 드슥을 켰는데, 아뿔싸! 분명 화면이 출력이 되어야할 드슥은 어디가고 심장멎은 심정지 환자가 있는게 아니겠는가!

![](https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTxSiEyE9WJNrR3-K7jnqELfhOmvooKoj6Ymw&s)

이런 슬픔을 공유하고 싶진 않기에 이 글을 작성하였다.
...
## 1. DCS가 설치되어있는 SSD 자체를 포맷시킨다.
   ...
   1-1. 당신의 DCS가 설치되어있는 SSD를 우클릭한다.

   1-2. 우클릭 하면 팝업창이 뜨게 되는데, 다른거 다 쌩 까고 *포맷...* 이라 적힌 칸을 누른다.

   1-3. 1-2를 다시 읽어보고 시작을 누른다.
